...
public class GrupoResponseDtoToGrupoEntity {
        ...
        public GrupoEntity responseToEntoty(GrupoResponseDto responseDto) {
                ...
                if (responseDto.getMaterias().size() > 0) {
                        responseDto.getMaterias().forEach(materia -> materiaEntities.add(materiaDao.findById(materia)
                                        .orElseThrow(() -> new ExcepcionSinDatos("No se encontro el aula"))));
                ...
                if (responseDto.getEstudiantes().size() > 0) {
                        responseDto.getEstudiantes().forEach(estudiante -> estudianteEntities
                                        .add(estudianteDao.findById(estudiante).orElseThrow(() -> new ExcepcionSinDatos(
                                                        "No se encontro el estudiante: " + estudiante))));

                } else {

                }

                responseDto.getEstrategias().forEach(estrategia -> {
                        if (estrategia > 0) {
                                estrategiaDao.findById(estrategia).orElseThrow(() -> new ExcepcionSinDatos(
                                                "No se encontro la estrategia '" + estrategia + "'."));
